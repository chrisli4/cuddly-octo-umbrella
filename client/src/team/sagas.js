...
function* teamSendFlow(action) {
	try {
		...
		const URL = `/users/${user.username}/team/send`
		const found = yield call(fetchAPI, URL, options('POST', user, { userB: userToSend }))
...
function* teamSendCancelFlow(action) {
	try {
		...
		const URL = `/users/${user.username}/team/cancel`
		const found = yield call(fetchAPI, URL, options('POST', user, { userB: userToCancel }))
...
function* teamAcceptFlow(action) {
	try {
		...
		const URL = `/users/${user.username}/team/accept`
		const accept = yield call(fetchAPI, URL, options('POST', user, { userB: userToAccept }))
...
function* teamDeclineFlow(action) {
	try {
		...
		const URL = `/users/${user.username}/team/decline`
		const decline = yield call(fetchAPI, URL, options('POST', user, { userB: userToDecline }))
...
function* teamRemoveFlow(action) {
	try {
		...
		const URL = `/users/${user.username}/team/remove`
		const removed = yield call(fetchAPI, URL, options('POST', user, { userB: userToRemove }))
